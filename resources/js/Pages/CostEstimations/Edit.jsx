...
const CAT_COLORS = { material: 'bg-blue-500', labor: 'bg-amber-500', subcontractor: 'bg-violet-500', transport: 'bg-teal-500', overhead: 'bg-slate-500', contingency: 'bg-rose-500', other: 'bg-gray-500' };
...
export default function CostEstimationEdit({ estimation, clients = [], leads = [], projects = [], serviceCategories = {} }) {
    const { data, setData, put, processing, errors } = useForm({
        title: estimation.title ?? '',
        service_group: estimation.service_group ?? '',
        ...
        items: (estimation.items ?? []).map(i => ({
            ...
            quantity: i.quantity, estimated_rate: i.estimated_rate, actual_total: i.actual_total ?? '', _key: Math.random(),
        })),
    });
    ...
    const grouped = useMemo(() => {
        const m = {};
        data.items.forEach((item, idx) => { const c = item.category || 'other'; if (!m[c]) m[c] = []; m[c].push({ ...item, _idx: idx }); });
        return m;
    }, [data.items]);

    const totalEstimated = useMemo(() => data.items.reduce((s, i) => s + (parseFloat(i.quantity) || 0) * (parseFloat(i.estimated_rate) || 0), 0), [data.items]);
    const markupAmount = totalEstimated * (parseFloat(data.markup_pct) || 0) / 100;
    const suggestedQuote = totalEstimated + markupAmount;

    const catTotals = useMemo(() => {
        const m = {};
        data.items.forEach(i => { const c = i.category || 'other'; m[c] = (m[c] || 0) + (parseFloat(i.quantity) || 0) * (parseFloat(i.estimated_rate) || 0); });
        return m;
    }, [data.items]);

    function updateItem(idx, field, val) { const next = [...data.items]; next[idx] = { ...next[idx], [field]: val }; setData('items', next); }

    function selectMaterialForItem(idx, materialId) {
        const mat = availableMaterials.find(m => m.id === materialId);
        if (!mat) { updateItem(idx, 'material_id', ''); return; }
        const next = [...data.items];
        next[idx] = { ...next[idx], material_id: mat.id, category: mat.category, description: mat.name, unit: mat.unit, estimated_rate: mat.default_rate };
        setData('items', next);
    }
    ...
    function submit(e) { e.preventDefault(); put(route('cost-estimations.update', estimation.id)); }

    return (
        <AppLayout>
            <Head title={`Edit ${estimation.code}`} />
            <PageHeader title={`Edit: ${estimation.code}`} subtitle={estimation.title} back={route('cost-estimations.show', estimation.id)} />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <FormField label="Service Type" error={errors.service_type} required>
                            <select className="form-input" value={data.service_type}
                                ...
                                <option value="">{data.service_group ? 'Select…' : 'Select category first'}</option>
                        ...
                        <FormField label="Markup %" error={errors.markup_pct}>
                                ...
                                <div key={cat} className="border border-gray-200 rounded-xl overflow-hidden">
                                    <div className="bg-gray-50 px-4 py-2.5 flex items-center justify-between border-b border-gray-200">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <button type="button" onClick={() => addItem(cat)} className="text-xs text-primary-600 flex items-center gap-1">
                                                    ...
                                                    <tr key={item._key} className="hover:bg-gray-50/50 group">
                                                        <td className="px-3 py-1.5">
                                                            <select className="form-input text-xs w-full"
                                                                ...
                                                                {categoryMaterials.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
                                                            </select>
                                                        </td>
                                                        <td className="px-3 py-1.5">
                                                        ...
                                                        <td className="px-3 py-1.5 text-right font-medium text-gray-700 text-xs">{total > 0 ? fmt(total) : '—'}</td>
                                                        <td className="px-3 py-1.5">
                ...
                <div className="card p-6">
                    <div className="flex justify-end">
                        <div className="w-80 space-y-2">
                            <div className="flex justify-between text-sm"><span className="text-gray-500">Total Cost</span><span className="font-semibold">{fmt(totalEstimated)}৳</span></div>
                            <div className="flex justify-between text-sm"><span className="text-gray-500">Markup ({data.markup_pct || 0}%)</span><span className="text-green-600">+ {fmt(markupAmount)}৳</span></div>
                            <div className="flex justify-between pt-2 border-t-2 border-gray-200"><span className="font-bold">Suggested Quote</span><span className="text-xl font-bold text-primary-700">{fmt(suggestedQuote)}৳</span></div>
                        </div>
                    </div>
                </div>

                <div className="flex gap-3 pb-6">
                    <button type="submit" disabled={processing} className="btn btn-primary">{processing ? 'Saving…' : 'Save Changes'}</button>
                    <a href={route('cost-estimations.show', estimation.id)} className="btn">Cancel</a>
                </div>
            </form>
        </AppLayout>
    );
