...
import {
    PlusIcon, PencilIcon, TrashIcon, MagnifyingGlassIcon, Squares2X2Icon,
} from '@heroicons/react/24/outline';

export default function InventoryCategoriesIndex({ categories, filters = {} }) {
    ...
    function submit(e) {
        e.preventDefault();
        if (ui.editCategory) {
            form.put(route('inventory.categories.update', ui.editCategory.id), {
                preserveScroll: true,
    ...
    function destroy(cat) {
        if (cat.items_count > 0) {
            alert(`Cannot delete — ${cat.items_count} inventory item${cat.items_count > 1 ? 's' : ''} are still using this category. Reassign them first.`);
            return;
        }
    ...
    function applySearch(e) {
        e.preventDefault();
        router.get(route('inventory.categories.index'), { search: ui.search }, {
    ...
    return (
        <AppLayout>
            ...
            <PageHeader title="Inventory Categories" subtitle="Classify items for easier searching and reports" back={route('inventory.items.index')}>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {rows.length === 0 && (
                                <tr><td colSpan={6} className="px-4 py-10 text-center text-gray-400">
                                    <Squares2X2Icon className="w-8 h-8 mx-auto text-gray-300 mb-2" />
                            ...
                            {rows.map(c => (
                                <tr key={c.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3 text-center">
                                        {c.items_count > 0 ? (
                                            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-blue-50 text-blue-700">{c.items_count}</span>
                                        ) : (
                ...
                <form onSubmit={submit} className="p-4 sm:p-6 space-y-4">
                    <FormField label="Name" required error={form.errors.name}>
                        <input type="text" className="form-input"
                            ...
                            placeholder="e.g. Furniture, Fabric, Lighting" autoFocus />
                    </FormField>
